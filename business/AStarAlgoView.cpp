#include "AStarAlgoView.h"

AStarAlgoView::AStarAlgoView()
{
    //ctor
}
...
const TCHAR *AStarAlgoView::getName() const {
    static const TCHAR *name = TEXT("A*");
    return name;
}

void AStarAlgoView::execute(const Board &board, wxTextCtrl &textCtrl, bool isSearchLogShown)
{
    logStr = _T("");
    nIterations = 0;
    solutionMap.clear();
    ...
    do
    {
        ...
        vector< AStar<Board>::Node * >::iterator iter;
        for (iter = astarsearch.openList.begin(); iter != astarsearch.openList.end(); ++iter) {
            AStar<Board>::Node *node = (*iter);
        ...
        for (iter = astarsearch.closedList.begin(); iter != astarsearch.closedList.end(); ++iter) {
            AStar<Board>::Node *node = (*iter);
        ...
        if (searchState == AStar<Board>::SEARCH_STATE_SUCCEEDED) {
            astarsearch.FreeUnusedNodes();
        }
    }
    while( searchState == AStar<Board>::SEARCH_STATE_SEARCHING );

    Board *prevNode = NULL;
    if( searchState == AStar<Board>::SEARCH_STATE_SUCCEEDED )
    {
        Board *node = astarsearch.GetSolutionStart();
        int steps = 0;
        for( ;; )
        {
            if (prevNode != NULL) {
...
void AStarAlgoView::logSearch(wxTextCtrl &textCtrl)
{
    wxString stepStr;
    stepStr += _T("\nSearch steps:\n");
    //WriteText(_T("\n"));
...
//int AStarAlgoView::getNIterations() {
//    return astarsearch.GetStepCount();
//}
